...
class Generator(tf.keras.Model):
    def __init__(self, img_size=256, img_ch=3, style_dim=64, max_conv_dim=512, sn=False, name='Generator'):
        ...
        self.to_rgb = Sequential(
            [
                ...
                Leaky_Relu(alpha=0.2),
                ##Conv(channels=self.img_ch, kernel=1, stride=1, sn=self.sn, name='to_rgb')
                Conv3D(channels=self.img_ch, kernel=1, stride=1, sn=self.sn, name='to_rgb')
            ]
        )
    ...
    def architecture_init(self):
        ...
        for i in range(self.repeat_num):
            ...
            decoder.insert(0, AdainResBlock(ch_out, ch_in, upsample=True, sn=self.sn,
                                            name='decoder_up_adaresblock_' + str(i)))  # stack-like        ##peter
    ...
    def call(self, x_init, training=True, mask=None):
        x, x_s = x_init
        # print('input of from_rgb')
        # print(x.shape)

        x = self.from_rgb(x)
        # print('ouput of from_rgb')
        # print(x.shape)

        for encoder_block in self.encoder:
            # print('input of from encoder_block')
            # print(x.shape)
            x = encoder_block(x)
            # print('ouput  of from encoder_block')
            # print(x.shape)
        for decoder_block in self.decoder:
            # print('input of from decoder_block')
            # print(x.shape)
            x = decoder_block([x, x_s])
            # print('ouput  of from decoder_block')
            # print(x.shape)

        # print('input of to_rgb')
        # print(x.shape)
        x = self.to_rgb(x)
        # print('ouput of to_rgb')
        # print(x.shape)
        return x
...
class MappingNetwork(tf.keras.Model):
    ...
    def architecture_init(self):
        layers = []
        layers += [FullyConnected(units=self.hidden_dim, sn=self.sn, name='shared_fc')]
        layers += [Relu(name='relu_0')]

        for i in range(3):
            layers += [FullyConnected(units=self.hidden_dim, sn=self.sn, name='shared_fc_' + str(i))]
            layers += [Relu(name='relu_MappingNetwork_' + str(i))]  ##peter
        ...
        for n_d in range(self.num_domains):
            for i in range(3):
                ...
                layers += [Relu(name='relu_MappingNetwork_' + str(n_d) + '_' + str(i))]  ##peter
            layers += [FullyConnected(units=self.style_dim, sn=self.sn, name='domain_{}_style_fc'.format(n_d))]
...
class StyleEncoder(tf.keras.Model):
    ...
    def architecture_init(self):
        ...
        ## blocks += [Conv(ch_in, kernel=3, stride=1, pad=1, sn=self.sn, name='init_conv')]
        blocks += [Conv3D(ch_in, kernel=3, stride=1, pad=1, sn=self.sn, name='init_conv')]
        ...
        ##blocks += [Conv(channels=ch_out, kernel=4, stride=1, pad=0, sn=self.sn, name='conv')]
        blocks += [Conv3D(channels=ch_out, kernel=4, stride=1, pad=0, sn=self.sn, name='conv')]
...
class Discriminator(tf.keras.Model):
    ...
    def architecture_init(self):
        ...
        blocks += [Leaky_Relu(alpha=0.2, name='relu_0')]  # add name --PETER
        blocks += [Conv3D(channels=ch_out, kernel=4, stride=1, pad=0, sn=self.sn, name='conv_0')]

        blocks += [Leaky_Relu(alpha=0.2, name='relu_1')]  # add name --PETER
        blocks += [Conv3D(channels=self.num_domains, kernel=1, stride=1, sn=self.sn, name='conv_1')]
