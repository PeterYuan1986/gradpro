...
class Image_data:
    ...
    #peter
    def image_to_tf(self,itk_img):
        itk_img = resize(itk_img, self.img_width, self.img_height, self.img_depth)  ###peter
    ...
    def image_processing(self, img, img2, domain):
       ...
       #
       # ## x = tf.io.read_file(filename)
       #  x_decode=tf.convert_to_tensor(x)
       ...
       #  img = preprocess_fit_train_image(x_decode)
       #  #x = tf.io.read_file(filename2)
       #  itk_img = sitk.ReadImage(filename2)
       #  itk_img = resize(itk_img, self.img_width, self.img_height, self.img_depth)  ###peter
       #  img2 = sitk.GetArrayFromImage(itk_img)
       #
       #  ##x = tf.io.read_file(filename2)
       #  x_decode2 = tf.convert_to_tensor(x)
       #  ##x_decode = tf.image.decode_jpeg(x, channels=self.channels, dct_method='INTEGER_ACCURATE')
       #  #x_decode2 = tf.image.resize(x_decode, [self.img_height, self.img_width])
       #  img2 = preprocess_fit_train_image(x_decode2)
    ...
    def preprocess(self):
        ...
        for idx, domain in enumerate(self.domain_list):
            image_list = glob(os.path.join(self.dataset_path, domain) + '/*.nii.gz') + glob(os.path.join(self.dataset_path, domain) + '/*.nii')            ##----Peter
            shuffle_list = random.sample(image_list, len(image_list))       #Return a k length list of unique elements chosen from the population sequence or set. Used for random sampling without replacement.
        ...
        for idx, path in enumerate(self.images):
                # if self.augment_flag:
                #     image_list[idx] = self.image_to_tf(augmentation(sitk.ReadImage(path)))
                # else:
                imageread=sitk.ReadImage(path)
                self.images[idx]= self.image_to_tf(imageread)

        for idx, path in enumerate(self.shuffle_images):
                # if self.augment_flag:
                #     shuffle_list[idx] = self.image_to_tf(augmentation(sitk.ReadImage(path)))
                # else:
                    self.shuffle_images[idx] = self.image_to_tf(sitk.ReadImage(path))


def adjust_dynamic_range(images, range_in, range_out, out_dtype):          #preprocess, change 255 into -1 to 1.
...
def load_images(image_path, img_size, img_channel=1):
    ...
    itk_img = resize(itk_img, img_size, img_size, img_size)  ###peter
...
def augmentation(img):
    ...
    flipped_transform = sitk.AffineTransform(3)
...
def load_test_image(image_path, img_size, img_channel=3):
    # peter
    # if img_channel == 1 :
...
def resize(img, dx,dy,dz):
    new_x_size = dx  # upsample
    new_y_size = dy  # downsample
    new_z_size = dz  # downsample
