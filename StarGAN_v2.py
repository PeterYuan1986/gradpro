...
class StarGAN_v2():
    ...
    def build_model(self):
        if self.phase == 'train':
            ...
            dataset_num = len(img_class.images)  #all the images with different domain
         ...
         #  img_and_domain = img_and_domain.map(map_func=img_class.image_processing, num_parallel_calls=AUTOTUNE)
            ...
            x = np.ones(shape=[self.batch_size, self.img_size,self.img_size, self.img_size, self.img_ch], dtype=np.float32)
...
#the generated image are 3d-image, it can't be shown on canvas.   peter
    def refer_canvas(self, x_real, x_ref, y_trg, path, img_num):   # self.refer_canvas(src_img, ref_img, ref_img_domain, save_path, img_num=merge_size)
        ...
        for col, image in enumerate(list(row_fake_images)):
            ...
            new_image = nib.Nifti1Image(np.uint8(image), affine=np.eye(4))
                #
    ...
    def latent_canvas(self, x_real, path):
        ...
        for row in range(self.num_style):
            ...
            for col, y_trg in enumerate(list(domain_fix_list)):
                ...
                new_image = nib.Nifti1Image(np.uint8(x_fake[0]), affine=np.eye(4))
                name =self.domain_list[col]
                x_path=path+name+'.nii'
                nib.save(new_image, x_path)
    ...
    def test(self, merge=True, merge_size=0):
        ...
        source_images = glob(os.path.join(source_path, '*.nii')) + glob(os.path.join(source_path, '*.nii.gz'))
        ...
        for idx, domain in enumerate(self.domain_list):
            image_list = glob(os.path.join(reference_path, domain) + '/*.nii') + glob(
                os.path.join(reference_path, domain) + '/*.nii.gz')
        ...
        #
        #             src_img_ = load_images(src_img_path, self.img_size, self.img_ch)  # [img_size, img_size,img_size, img_ch]     peter
        ...
        #
        #         # the generated image are 3d-image, it can't be shown on canvas.   peter
        #         # save_path = './{}/ref_all.nii'.format(self.result_dir)
        ...
        for src_img_path in tqdm(source_images):
            ...
            for ref_img_path, ref_img_domain in zip(reference_images, reference_domain):
                ...
                ref_img = load_images(ref_img_path, self.img_size, self.img_ch)  # [img_size, img_size, img_size, img_ch]
                ref_img = tf.expand_dims(ref_img, axis=0)
                ref_img_domain = tf.expand_dims(ref_img_domain, axis=0)
                dir_path = './{}/ref/'.format(self.result_dir)
                if not os.path.exists(dir_path):
                    os.makedirs(dir_path)
                save_path = './{}/ref/ref_{}_{}{}'.format(self.result_dir, src_name, ref_name, src_extension)
        ...
        for src_img_path in tqdm(source_images):
            ...
            src_img = load_images(src_img_path, self.img_size, self.img_ch)  # [img_size, img_size, img_ch]
            src_img = tf.expand_dims(src_img, axis=0)
            dir_path='./{}/latent/'.format(self.result_dir)
            if not os.path.exists(dir_path):
                os.makedirs(dir_path)
